...
// : 동물과 같이 구체적인 실체라기보다는 실체들의 공통되는 특성을 가지고 있는 추상적인 것!
// abstract class는 실체 클래스가 공통적으로 가져야 할 필드와 메서드들을 정의해 놓은 추상적인 클래스
// => 실체 클래스의 멤버(필드, 메서드)를 통일화하는데 목적!

// - 추상클래스와 일반클래스는 상속의 관계를 가짐
// - 추상클래스는 부모, 일반클래스(실체) 자식으로 구현되어 
//   실체클래스는 추상클래스의 모든 특성을 물려받고, 추가적인 특성을 가질 수 있다.
...
// 2. 추상클래스는 일반변수, 추상메서드, 일반메서드, 생성자 가질 수 있음
...
//    메서드명도 이미 정해져 있기 때문에 관리가 편함(실무에서는 abstract보다는 인터페이스 선호)
// 8. 추상클래스와 일반클래스 중 어떤 것이 좋다고 정답 없음(다만 강제성이 필요할 때 abstratc 사용)
...
public abstract class Animal{
	...
	// 추상함수(추상메서드)
	//  - 실행부{} 없음
	//  - 반드시 자식클래스가 오버라이딩해서 사용(강제성)
	//  - 접근제한자는 public, protected, default만 사용 가능(private 금지)
